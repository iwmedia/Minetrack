          ...
          if (err) {
            ...
            const payload = {
              players: {
                ...
                max: sanitizeMaxPlayers(res.players.max)
              },
        ...
        if (err) {
          ...
          callback(null, {
            players: {
              ...
              max: sanitizeMaxPlayers(res.maxPlayers)
            }
          })
        }
      }, timeout)
...
function getPingErrorCause (err) {
  const cause = err.code || err.errno || err.message || 'unknown'

  return String(cause).substring(0, 64)
}
...
class PingController {
  ...
  pingAll = () => {
    ...
    this.startPingTasks(results => {
      ...
      for (const serverRegistration of this._app.serverRegistrations) {
        ...
        if (config.logToDatabase) {
          pingLogEntries.push({
            ...
            onlinePlayers: getPlayerCountOrNull(result.resp),
            maxPlayers: result.resp && result.resp.players ? result.resp.players.max : undefined,
            error: result.err ? getPingErrorCause(result.err) : undefined
          })
        }
      ...
      if (config.logToDatabase && pingLogEntries.length > 0) {
        this._app.database.insertPings(timestamp, pingLogEntries)
      }
