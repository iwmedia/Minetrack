...
class ServerRegistration {
  ...
  getUpdate (timestamp, resp, err, version) {
    ...
    if (resp) {
      ...
      if (config.logToDatabase && (!this.recordData || resp.players.online > this.recordData.playerCount)) {
        ...
        this._app.database.updatePlayerCountRecord(this.data.name, resp.players.online, timestamp)
      }
  ...
  updateProtocolVersionCompat (incomingId, outgoingId, protocolIndex) {
    // A pinned protocol that is not in the version list has no index to label
    if (protocolIndex < 0) {
      return false
    }
  ...
  getNextProtocolVersion () {
    ...
    const protocolVersions = minecraftVersions.get()[this.data.type]

    // Servers whose proxy resets unknown protocol ids can pin one via the
    // "pinnedProtocol" field in their servers document; this skips the
    // protocol cycling (and with it the supported-version detection)
    if (Number.isInteger(this.data.pinnedProtocol)) {
      return {
        protocolId: this.data.pinnedProtocol,
        protocolIndex: protocolVersions.findIndex(version => version.protocolId === this.data.pinnedProtocol)
      }
    }
