...
class Server {
  ...
  handleHistoryRequest = (req, res) => {
    ...
    const maxWindowMillis = unitConfig.maxDays * 24 * 60 * 60 * 1000

    const respond = (fromMillis, toMillis, cacheKey) => {
      if (cacheKey) {
    ...
    // Exact window via from/to (epoch millis), used by the day range picker.
    // User-picked one-offs, served uncached.
    const fromParam = url.searchParams.get('from')
    const toParam = url.searchParams.get('to')

    if (fromParam !== null || toParam !== null) {
      const fromMillis = Number(fromParam)
      const toMillis = Number(toParam)

      if (!Number.isInteger(fromMillis) || !Number.isInteger(toMillis) || fromMillis >= toMillis) {
        sendJson(400, { error: '"from" and "to" must be epoch milliseconds with from < to' })
        return
      }

      if (toMillis - fromMillis > maxWindowMillis) {
        sendJson(400, { error: `Window too large, at most ${unitConfig.maxDays}d for unit "${unit}"` })
        return
      }

      respond(fromMillis, toMillis)
      return
    }

    // Trailing window via range=Nd; the default views, cached briefly since
    // rollups only change once per maintenance run
    ...
    respond(toMillis - rangeDays * 24 * 60 * 60 * 1000, toMillis, `${unit}|${rangeDays}`)
  }
